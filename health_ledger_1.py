...
def verifier_input():
    return int(input("Hey Verifier!\nEnter a random bit 0 or 1\n"))

def prover_input():
    prime=int(input("Hey Prover!\nEnter a prime number:\n"))  
    random=int(input("Hey Prover!\nEnter a random number b/w 2 and(prime -1)\n"))
...
def createBlock(obj):
    ...
    #Uncomment below lines to take console input
    '''
    usr_name = f_list[4*i].strip('\n')
    usr_med_rep = f_list[4*i+1].strip('\n')
    usr_permitted = f_list[4*i+2].strip('\n')
...
def main():
    ...
    while(exit_flag!=True):
        ...
        if(usr_in == '1'):
            ...
            exit_flag=True
            #print(dir(obj[0]))
            print("Thank you for utilizing the ledger\n_/\_")
        else:
